...
const { mockGetServerSession, mockGetGatewayAuthLevel, mockFindMany } = vi.hoisted(() => ({
...
describe("/api/user/search route", () => {
  ...
  it("returns 401 when session is missing", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/user/search?q=test"));
    expect(res.status).toBe(401);
  });

  it("returns empty items when query is blank", async () => {
    mockGetServerSession.mockResolvedValue({ user: { email: "x@example.com" } });
    mockGetGatewayAuthLevel.mockResolvedValue({ isOfficer: false });
    const res = await GET(new Request("http://localhost/api/user/search?q=   "));
  ...
  it("hides email for non-officers in search results", async () => {
    mockGetServerSession.mockResolvedValue({ user: { email: "x@example.com" } });
    mockGetGatewayAuthLevel.mockResolvedValue({ isOfficer: false });
    mockFindMany.mockResolvedValue([{ id: 1, name: "User", email: "user@example.com" }]);
    const res = await GET(new Request("http://localhost/api/user/search?q=user"));
