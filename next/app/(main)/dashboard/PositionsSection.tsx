...
export default function PositionsSection({
  ...
  const [removeOfficerPosition, setRemoveOfficerPosition] =
  ...
  const getPendingInvitation = (
    ...
    return (
      pendingInvitations.find((inv) => inv.positionId === positionId) || null
    );
  };
  ...
  const handleSendSwipeAccess = async (
    positionsToSend: Position[],
    label: string
  ) => {
    const people = positionsToSend
      .map((position) => position.currentOfficer)
      .filter(
        (officer): officer is NonNullable<Position["currentOfficer"]> =>
          !!officer
      )
      ...
      render: (position) => {
        ...
        return (
          <OfficerAssignmentCard
            ...
            onCancelInvitation={() =>
              pendingInv && setCancelInvitation(pendingInv)
            }
            readOnly={readOnly}
          />
        );
    ...
    },
    {
      ...
      render: (position) => (
        <Button
          ...
          variant="outline"
          onClick={() =>
            router.push(`/dashboard/positions/${position.id}/handover`)
          }
            ...
            render: (position: Position) => (
              ...
              >
                ...
                <Button
                  ...
                  disabled={
                    position.isFilled || !!getPendingInvitation(position.id)
                  }
                  title={
                    position.isFilled
                      ? "Cannot delete position with assigned officer"
                      : getPendingInvitation(position.id)
                        ? "Cannot delete position with pending invitation"
                        : "Delete position"
                  }
                  className={
                    isMobile
    ...
    <div className="space-y-6">
      ...
      >
        <p className="text-sm text-muted-foreground">
          ...
          <strong>{deletePosition?.title}</strong>? This action cannot be
          undone.
        </p>
        <ModalFooter>
      ...
      >
        <p className="text-sm text-muted-foreground">
          Remove <strong>{removeOfficerPosition?.currentOfficer?.name}</strong>{" "}
          from <strong>{removeOfficerPosition?.title}</strong>?
        </p>
        <ModalFooter>
          <Button
            variant="ghost"
            onClick={() => setRemoveOfficerPosition(null)}
          >
            Cancel
          </Button>
          <Button
      ...
      >
        <p className="text-sm text-muted-foreground">
          Send a swipe access request for the currently assigned{" "}
          {swipeModalLabel.toLowerCase()}?
        </p>
        <ModalFooter>
