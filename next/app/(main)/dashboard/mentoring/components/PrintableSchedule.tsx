    ...
    try {
      ...
      if (scheduleId) {
        ...
        if (res.ok) {
          ...
          scheduleData =
            schedules.find((s: MentorSchedule) => s.isActive) || null;
        }
      }
      ...
      if (scheduleData) {
        ...
        const blocksRes = await fetch(
          `/api/scheduleBlock?scheduleId=${scheduleData.id}`
        );
        if (blocksRes.ok) {
    ...
    <>
      ...
      >
        <div
          ...
          className="p-4 print:p-0 w-full mx-auto print:h-screen print:flex print:flex-col"
        >
          {/* Header */}
          <div className="text-center mb-4 print:mb-0.5">
            <h1 className="text-2xl font-bold print:text-base print:leading-tight">
              SSE Mentor Schedule
            </h1>
            <p className="text-muted-foreground text-base print:text-[10px] print:leading-tight">
              {schedule.name}
            </p>
          </div>
          ...
          <div className="mt-3 print:mt-0.5 text-center text-xs print:text-[9px] text-muted-foreground">
            <p className="font-medium">
              Society of Software Engineers • Golisano Hall • sse.rit.edu
            </p>
          </div>
