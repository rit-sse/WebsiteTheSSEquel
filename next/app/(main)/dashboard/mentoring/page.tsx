...
export default function MentoringDashboardPage() {
  ...
  const handleTabChange = (nextTab: string) => {
    if (
      ...
      latestUnseenUpdateAt > 0
    ) {
      localStorage.setItem(
        "mentor-availability-last-seen",
        String(latestUnseenUpdateAt)
      );
  ...
  useEffect(() => {
    const checkMentorAvailabilityUpdates = async () => {
      try {
        ...
        const latestUpdatedAt = updates?.latestUpdatedAt
          ? Date.parse(updates.latestUpdatedAt)
          : 0;
        const seenAt = Number(
              ...
              (entry: {
                ...
                if (removedBlocks.length > 0) {
                  const labels = removedBlocks
                    ...
                    .map(
                      (block) =>
                        `${dayNames[(block.weekday ?? 1) - 1] ?? "Day"} ${block.startHour}:00`
                    )
                    .join(", ");
                  const suffix =
                    removedBlocks.length > 2
                      ? ` +${removedBlocks.length - 2} more`
                      : "";
                  toast.info(
                    `${name} updated availability and was removed from ${labels}${suffix}.`
                  );
                } else {
  ...
  const titleSuffix = activeSemesterName
    ? activeSemesterName.replace(
        /^([A-Za-z]+)\s+(\d{4})$/,
        (_, term: string, year: string) => {
          ...
          >
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2">
              <TabsList className="inline-flex w-auto gap-1">
                <TabsTrigger
                  value="schedule"
                  className="flex items-center gap-1.5 text-xs sm:text-sm px-2.5 sm:px-3"
                >
