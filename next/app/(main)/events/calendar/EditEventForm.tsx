    ...
    if (isOpen && event) {
      ...
      if (event.id) {
        fetch(`/api/event/${event.id}/purchases`)
          .then((r) => (r.ok ? r.json() : []))
          .then((data: LinkedPurchaseRequest[]) =>
            setLinkedPurchases(Array.isArray(data) ? data : [])
          )
          .catch(() => {});
        ...
        fetch("/api/purchasing")
          ...
          .then((data: AvailablePurchaseRequest[]) =>
            setAvailablePurchases(
              data.filter((p) => !p.eventId || p.eventId === event.id)
            )
          )
          .catch(() => {});
      }
  ...
  const handleLinkPurchase = async (purchaseId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAvailablePurchases((prev) =>
          prev.map((p) =>
            p.id === purchaseId ? { ...p, eventId: event.id ?? null } : p
          )
        );
        setShowLinkSelect(false);
      } else {
  ...
  const handleUnlinkPurchase = async (purchaseId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAvailablePurchases((prev) =>
          prev.map((p) => (p.id === purchaseId ? { ...p, eventId: null } : p))
        );
      } else {
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <Badge variant="outline" className="text-xs shrink-0">
                    ${parseFloat(p.estimatedCost).toFixed(2)}
                  </Badge>
                </div>
                <Button
                  type="button"
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-48 overflow-y-auto border rounded-md space-y-0.5">
              ...
              {availablePurchases.filter(
                (p) => !linkedPurchases.some((l) => l.id === p.id)
              ).length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-3">
                  No unlinked purchase requests found
                </p>
              )}
            </div>
            <Button
