  ...
  const fetchEvents = async () => {
    try {
      ...
      if (Array.isArray(res)) {
        const sorted = res.sort((event1, event2) =>
  ...
  return (
    <>
      <Card className="flex flex-col w-full max-w-[94vw] xl:max-w-[1400px] p-6 md:p-10 mx-auto mt-8">
        <h1 className="text-primary text-left mb-6">Events Calendar</h1>
      ...
      >
        <EventForm
          ...
          event={selectedEvent}
          openEditModal={() => setModalEdit(true)}
          events={events}
          setEvents={setEvents}
        />
      </Modal>
