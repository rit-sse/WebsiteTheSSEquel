...
const mapFunc = (item: golinkInterf) => ({
  id: item.id,
  goUrl: item.golink,
  url: item.url,
...
const GoLinksPage = () => {
  ...
  const fetchData = useCallback(async () => {
    const data: golinkInterf[] = await fetch("/api/golinks/public").then(
      (response) => response.json()
    );
    const authLevel = await fetch("/api/authLevel").then((resp) => resp.json());
    const isOfficer = authLevel["isOfficer"];
    if (isOfficer) {
      const officerData: golinkInterf[] = await fetch(
        "/api/golinks/officer"
      ).then((response) => response.json());
      setGoLinkData([...officerData.map(mapFunc), ...data.map(mapFunc)]);
    } else {
      setGoLinkData([...data.map(mapFunc)]);
    }
  }, []);
  useEffect(() => {
