...
const Projects = () => {
  ...
  const [projectsHeadEmail, setProjectsHeadEmail] = useState<string | null>(
    null
  );
    ...
    fetch("/api/officer")
      ...
      .then((officers: OfficerLookup[]) => {
        const projectsHead = officers.find(
          (officer) =>
            officer.is_active && officer.position.title === "Projects Head"
        );
  ...
  return (
    <section className="w-full py-8 px-4 md:px-8 lg:px-12">
          ...
          <div className="text-center mb-8">
            ...
            {isOfficer ? (
              <div className="flex justify-center mt-6">
                <button
                  className="bg-primary text-primary-foreground px-[25px] py-[10px] rounded-lg"
                  onClick={enableModal}
                >
                  Add Project
                </button>
      ...
      <AddProjectModal
        enabled={addProjectModalEnabled}
        setEnabled={setAddProjectModalEnabled}
      />
    </section>
  );
};
