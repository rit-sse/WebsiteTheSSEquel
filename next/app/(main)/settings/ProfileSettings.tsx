...
export default function ProfileSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const originalGraduationYear = original?.graduationYear
        ? String(original.graduationYear)
        : "";
      ...
      if (coopSummary !== (original?.coopSummary ?? ""))
        payload.coopSummary = coopSummary;
      if (image !== (original?.image ?? DEFAULT_IMAGE)) {
        ...
        if (image === DEFAULT_IMAGE) {
          ...
          payload.image = isS3Key(image)
            ? image
            : (original?.profileImageKey ?? image);
        }
      }
  ...
  const handleReset = () => {
    ...
    setGraduationYear(
      original.graduationYear ? String(original.graduationYear) : ""
    );
