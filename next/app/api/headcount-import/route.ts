  ...
  try {
    ...
    if (!canManage) {
      return NextResponse.json(
        {
          error:
            "Only Mentoring Head or Primary Officers can import headcount data",
        },
        { status: 403 }
      );
    }
    ...
    courses.forEach((course) => {
      const codeKey = normalizeCourseCode(
        `${course.department.shortTitle}${course.code}`
      );
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      const mentorIds = row.mentors
        .map((mentor) => {
          ...
          return (
            mentorByEmail.get(emailKey) ??
            mentorByName.get(normalizeKey(mentor))
          );
        })
    ...
    if (type === "mentor" && validated.length > 0) {
      ...
      const existingSet = new Set(
        existing.map(
          (e) =>
            `${e.createdAt.toISOString()}|${e.peopleInLab}|${e.feeling}|${e.semesterId}`
        )
      );
      ...
      const existingSet = new Set(
        existing.map(
          (e) =>
            `${e.createdAt.toISOString()}|${e.studentsMentoredCount}|${e.testsCheckedOutCount}|${e.semesterId}`
        )
      );
      for (const v of validated) {
      ...
      if (type === "mentor") {
        await prisma.mentorHeadcountEntry.create({
          data: {
            ...
            mentors:
              v.mentorIds.length > 0
                ? {
                    createMany: {
                      data: v.mentorIds.map((mentorId) => ({ mentorId })),
                    },
                  }
                : undefined,
          },
          ...
          data: {
            ...
            mentors:
              v.mentorIds.length > 0
                ...
                : undefined,
            classes:
              v.row.classes && v.row.classes.length > 0
                ? {
                    createMany: {
                      data: v.row.classes
  ...
  try {
    ...
    if (!canManage) {
      return NextResponse.json(
        {
          error:
            "Only Mentoring Head or Primary Officers can clear headcount data",
        },
        { status: 403 }
      );
    }
    ...
    const deleted = await prisma.$transaction(async (tx) => {
      ...
      return {
        ...
        joinRecords:
          mentorMentors.count + menteeMentors.count + menteeCourses.count,
      };
    });
