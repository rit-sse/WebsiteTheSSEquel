...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (when2meetUrl !== undefined)
      updateData.when2meetUrl = when2meetUrl || null;
    if (applicationOpen !== undefined)
      updateData.applicationOpen = applicationOpen
        ? new Date(applicationOpen)
        : null;
    if (applicationClose !== undefined)
      updateData.applicationClose = applicationClose
        ? new Date(applicationClose)
        : null;
    if (semesterStart !== undefined)
      updateData.semesterStart = semesterStart ? new Date(semesterStart) : null;
    if (semesterEnd !== undefined)
      updateData.semesterEnd = semesterEnd ? new Date(semesterEnd) : null;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (existingSemester._count.applications > 0) {
      ...
      console.log(
        `Deleting semester ${id} with ${existingSemester._count.applications} applications`
      );
    }
