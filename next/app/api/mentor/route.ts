...
export async function POST(request: Request) {
  ...
  if (!("expirationDate" in body && "isActive" in body && "mentorId" in body)) {
    return new Response(
      '"expirationDate", "isActive", and "mentorId" must be included in request body',
      { status: 400 }
    );
  }
  const expirationDate = body.expirationDate;
  const isActive = body.isActive;
  const user_Id = body.mentorId;
...
export async function DELETE(request: Request) {
  ...
  const mentor = await prisma.mentor.delete({ where: { id } });
