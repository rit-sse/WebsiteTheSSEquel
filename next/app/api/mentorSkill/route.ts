...
export async function POST(request: NextRequest) {
  ...
  if (!("mentor_Id" in body && "skill_Id" in body)) {
    return new Response(
      '"mentor_Id" and "skill_Id" must be included in request body',
      {
        status: 400,
      }
    );
...
export async function PUT(request: NextRequest) {
  ...
  if (!mentorSkill_exists) {
    return new Response(`Couldn't find mentorSkill with ID ${body.id}`, {
      status: 404,
    });
  }
  if ("mentor_Id" in body) {
    ...
    if (!mentor_exists) {
      return new Response(`Couldn't find mentor with ID ${body.mentor_Id}`, {
        status: 404,
      });
    }
  }
  if ("skill_Id" in body) {
    ...
    if (!skill_exists) {
      return new Response(`Couldn't find skill with ID ${body.skill_Id}`, {
        status: 404,
      });
    }
  }
