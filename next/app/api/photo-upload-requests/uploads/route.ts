...
export async function POST(request: NextRequest) {
  ...
  if (body.files.length > PHOTO_REQUEST_UPLOAD_LIMIT) {
    return NextResponse.json(
      {
  ...
  const requestToken = signBatchId(batchId);
  const uploads = [];
  ...
  for (const file of body.files) {
    ...
    const { originalKey, galleryKey } = buildPhotoRequestObjectKeys({
      batchId,
  ...
  return NextResponse.json({ batchId, requestToken, uploads });
}

function signBatchId(batchId: string) {
  const secret = process.env.NEXTAUTH_SECRET ?? "photo-upload-request-dev";
  const signature = createHmac("sha256", secret).update(batchId).digest("hex");
