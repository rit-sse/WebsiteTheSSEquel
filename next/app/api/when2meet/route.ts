  ...
  try {
    ...
    if (!match) {
      return NextResponse.json(
        {
          error:
            "Invalid When2Meet URL. Expected format: https://www.when2meet.com/?XXXXXXXX-XXXXX",
        },
        { status: 400 }
      );
    }
    ...
    if (!data) {
      return NextResponse.json(
        {
          error:
            "Failed to parse When2Meet data. The event may be expired or invalid.",
        },
        { status: 422 }
      );
    }
...
function parseWhen2MeetData(html: string): When2MeetData | null {
  try {
    // Extract event name
    const eventNameMatch = html.match(
      /<span id="NewEventNameDiv"[^>]*>([^<]+)<\/span>/
    );
    ...
    const availableAtSlotMatch = html.match(
      /AvailableAtSlot\s*=\s*\[([^\]]*)\]/
    );
