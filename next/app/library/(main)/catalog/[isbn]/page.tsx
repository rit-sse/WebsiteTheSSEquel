...
export default function BookPage({
  ...
  const [simpleData, setSimpleData] = useState<
    { id: number; checkedOut: boolean }[]
  >([]);
  useEffect(() => {
    fetch(
      (process.env.INTERNAL_API_URL ? process.env.INTERNAL_API_URL : "") +
        "/api/library/book?count=true&isbn=" +
        isbn
    )
      .then((resp) => resp.json())
      .then((data) => {
    ...
    fetch(
      (process.env.INTERNAL_API_URL ? process.env.INTERNAL_API_URL : "") +
        "/api/library/books?simple=true&isbn=" +
        isbn
    )
      .then((resp) => resp.json())
      .then((data) => {
    ...
    <div className="p-4 w-[80%]">
      <div className="flex flex-col md:flex-row gap-8 items-start w-full">
        <aside className="w-[260px]">
          {loaded ? (
            <Image
              ...
              height={380}
              className={
                "w-full h-auto object-cover rounded shadow-lg md:mt-[-60px] relative"
              }
            />
          ) : (
        ...
        <section className="w-full max-w-[980px]">
          {loaded ? (
            <>
              <h1 className="text-[44px] leading-[1.05] mb-1">
                {bookData.name}
              </h1>
              <p className="mt-1 text-[18px] text-gray-900">
                By {bookData.authors}
              </p>
              ...
              <div className="mt-5 text-base">
                <div className="mb-1">
                  <strong>Affiliated Courses:</strong>{" "}
                  <span className="ml-1">
                    {bookData.classInterest ?? "No class specified"}
                  </span>
                </div>
                <div className="mb-1">
                ...
                <div className="mb-1">
                  ...
                  <span className="ml-1">{bookData.authors ?? "Unknown"}</span>
                </div>
                <div className="mt-1">
                ...
                <div className="mt-1">
                  ...
                  <span className="ml-1">{bookData.edition ?? "Unknown"}</span>
                </div>
                <div className="mt-1">
                  <strong>Year Published:</strong>{" "}
                  <span className="ml-1">
                    {bookData.yearPublished ?? "Unknown"}
                  </span>
                </div>
                <div className="mt-1">
                  <strong>Keywords:</strong>{" "}
                  <span className="ml-1">{bookData.keyWords ?? "None"}</span>
                </div>
              </div>
              <table className="w-full border-collapse border border-black mt-[5px] [&_td]:px-[5px] [&_td]:py-[3px] ">
                <thead>
                  <tr className="mt-6 text-lg font-semibold ">
                    <td className="bg-gray-200 border-x border-black">
                      Book Id
                    </td>
                    <td className="bg-gray-200 border-x border-black text-right">
                      Checked Out Status
                    </td>
                  </tr>
                </thead>
                <tbody>
                  {simpleData.map((copy) => (
                    <tr key={copy.id} className="text-base">
                      ...
                      <td
                        className={
                          "border-x border-y border-black text-right" +
                          (copy.checkedOut
                            ? " text-red-600"
            ...
            <div className="w-full">
              ...
              <div className="mt-5 text-base w-full">
                {Array.from({ length: 7 }).map((_, index) => (
                  <div
                    key={index}
                    className="h-5 w-[50%] bg-gray-200 rounded mb-2 animate-pulse"
                  ></div>
                ))}
              </div>
            </div>
