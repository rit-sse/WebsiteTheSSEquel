  ...
  return (
    <div className="w-[80%] h-full relative flex items-center flex-col bg-white">
      ...
      {Object.keys(categories).length === 0 ? (
        ...
        Object.keys(categories).map((categoryKey) => (
          <FeaturedContainer
            key={categoryKey}
            props={{
              books: Array.isArray(categories[categoryKey]?.books)
                ? categories[categoryKey].books
                : [],
              header: categoryKey,
            }}
          />
      ...
      <div className="w-[100%] md:w-[60%] grid grid-cols-2 text-center gap-6 mt-5">
        <LibraryQuickLink
          props={{ label: "Browse All Books", link: "/library/catalog" }}
        />
        <LibraryQuickLink
          props={{ label: "Textbook Donations", link: "/library/donations" }}
        />
      </div>
    </div>
  );
