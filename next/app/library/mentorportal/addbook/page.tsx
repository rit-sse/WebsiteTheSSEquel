...
export default function AddBook() {
  ...
  const fetchISBNExists = () => {
    setError("");
    setBookExists(false);
    fetch("/api/library/book?count=true&isbn=" + isbnInput)
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
    ...
    <div className="w-[100%] flex flex-col items-center mb-10">
      <div className="flex justify-center items-center w-[100%]">
        <input
          ...
          placeholder="Find if ISBN exists in database"
          onChange={(e) => {
            setIsbnInput(e.target.value);
          }}
          onKeyUp={(e) => (e.key == "Enter" ? fetchISBNExists() : null)}
        />
        <Image
          src="/library-icons/search.png"
          alt="Search"
          className="w-[30px] h-[30px] ml-2 cursor-pointer"
          onClick={fetchISBNExists}
          width={30}
          height={30}
        />
      </div>
        ...
        <table className="w-[80%] max-w-[540px] border my-3 [&_td]:border [&_td]:p-2 bg-gray-100">
          ...
          <tr>
            ...
            <td>
              {bookData.ISBN}{" "}
              <Image
                className="w-[20px] h-[20px] inline cursor-pointer"
                src="/library-icons/information.png"
                alt="Book"
                onClick={() =>
                  (location.href = "/library/catalog/" + bookData.ISBN)
                }
                width={20}
                height={20}
              />
            ...
            <td className=" text-right border-none max-w-[50px]">
              <button
                onClick={addExistingBook}
                className="px-3 from-sky-800 to-sky-500 bg-gradient-to-b text-white font-bold rounded hover:to-sky-400 hover:from-sky-700 italic"
              >
                Add
              </button>
            </td>
      ...
      <div className="py-4 flex items-center">
        <p>Or</p>
        <button
          className=" ml-3 px-3 py-2 from-sky-800 to-sky-500 bg-gradient-to-b text-white font-bold rounded hover:to-sky-400 hover:from-sky-700 italic"
          onClick={() => {
            setBookExists(false);
            setRegisterNewPrompt(true);
          }}
        >
          Register New
        </button>
      ...
      {registerNewPrompt ? (
        <div className="w-[80%] border p-4">
          ...
          <div className="w-full flex flex-row justify-center items-center mb-4 ">
            <input
              type="text"
              placeholder="Attempt auto-fill with ISBN using OpenLibrary"
              className="w-[40%] min-w-[250px]"
              onChange={(e) => setLookupISBNInput(e.target.value)}
              onKeyUp={(e) => (e.key == "Enter" ? lookupISBN() : undefined)}
            />
            <Image
              src="/library-icons/search-web.png"
              alt="Search"
              className="h-full ml-4 cursor-pointer inline"
              onClick={lookupISBN}
              width={30}
              height={30}
            />
            <button
              className=" ml-3 px-3 py-2 from-gray-200 to-gray-300 bg-gradient-to-b text-black border hover:from-gray-100 hover:to-gray-200 "
              onClick={lookupOnIsbnSearch}
            >
              See in ISBN Search
            </button>
          </div>
          <div className="flex flex-col gap-3">
            ...
            <input
              ...
              onChange={(e) =>
                setNewBookData({ ...newBookData, authors: e.target.value })
              }
            />
            <input
              ...
              onChange={(e) =>
                setNewBookData({ ...newBookData, publisher: e.target.value })
              }
            />
            <input
              placeholder="Edition"
            ...
            <input
              ...
              type="number"
              className="p-2 border"
              value={newBookData.yearPublished}
              onChange={(e) =>
                setNewBookData({
                  ...newBookData,
            ...
            <input
              ...
              className="p-2 border"
              value={newBookData.keyWords}
              onChange={(e) =>
                setNewBookData({ ...newBookData, keyWords: e.target.value })
              }
            />
            <input
              placeholder="Class Interest"
              className="p-2 border"
              value={newBookData.classInterest}
              onChange={(e) =>
                setNewBookData({
                  ...newBookData,
                  classInterest: e.target.value,
                })
              }
            />
            <input
            ...
            />
            <button
              className="px-3 from-green-600 to-green-700 bg-gradient-to-t text-white font-bold border border-solid border-black hover:from-green-500 hover:to-green-600 rounded mt-3"
              onClick={uploadBook}
            >
              Add
            </button>
