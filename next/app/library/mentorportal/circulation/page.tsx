...
export default function CirculationPage() {
  ...
  async function circulationLookup() {
    await fetch(
      "/api/library/copies?" +
        (selectFilter === "isbn" ? "isbn=" + isbnLookup : "id=" + isbnLookup)
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
    ...
    <div className="w-[100%] px-5">
      ...
      <div>
        ...
        <input
          ...
          onKeyUp={(e) => (e.key === "Enter" ? circulationLookup() : null)}
        />
        <button
          className=" ml-2 px-2 from-gray-200 to-gray-300 bg-gradient-to-b text-black border hover:from-gray-100 hover:to-gray-200 "
          onClick={circulationLookup}
        >
          Search
        </button>
      </div>
          ...
          {searchResults.map((copy, index) => (
            <tr key={index}>
              ...
              <td>
                <input
                  type="checkbox"
                  checked={copy.checkedOut}
                  onChange={(e) => setAvailable(copy.id, !e.target.checked)}
                />
              </td>
            </tr>
          ))}
