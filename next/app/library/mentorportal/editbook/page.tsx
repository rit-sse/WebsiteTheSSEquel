...
export default function EditBook() {
  ...
  const fetchISBN = () => {
    ...
    fetch("/api/library/book?isbn=" + isbnInput)
      .then((res) => res.json())
      .then((data) => {
        setLoading(false);
        if (data.error) {
  ...
  const requestDelete = () => {
    if (
      !confirm(
        "Are you sure you want to delete this book? **This action cannot be undone.** All copies will also be removed"
      )
    ) {
      return;
    }
  ...
  const getDisplayImage = () => {
    ...
    return bookData.image + sep + "t=" + imageVersion;
  };
    ...
    <>
        ...
        <div className="p-4 w-fit min-w-[300px] md:min-w-[800px] mx-auto mt-4 border border-gray-300 rounded-md flex [&_input]:w-full ">
          ...
          >
            ...
            <div className="mt-3 flex items-center cursor-pointer">
              <Image
                src="/library-icons/openfile.png"
                alt="Search"
                className="w-[25px] h-[25px] ml-2 cursor-pointer"
                width={25}
                height={25}
              />
              <p className="pl-2 text-blue-600 underline hover:text-blue-800">
                Change Image
              </p>
            </div>
          </div>
          <div className="[&_div]:flex  [&_div]:items-center [&_div]:mt-2 [&_input]:py-[3px] [&_input]:ml-3 pl-5 w-[80%]">
            <div>
              ...
              <input
                value={bookData.name}
                onChange={(e) => editBook("name", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => editBook("authors", e.target.value)}
              />
            </div>
            <div>
              <strong>ISBN:</strong>{" "}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => editBook("publisher", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => editBook("edition", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                type="number"
                onChange={(e) => editBook("yearPublished", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => editBook("description", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => editBook("keyWords", e.target.value)}
              />
            </div>
            <div>
              <strong>Class Interest:</strong>{" "}
              <input
                value={bookData.classInterest}
                onChange={(e) => editBook("classInterest", e.target.value)}
              />
            </div>
            <div className="flex justify-between [&_button]:ml-3 [&_button]:px-4 [&_button]:py-2 [&_button]:rounded-md  mt-4">
              <div>
                <button
                  className="from-red-600 to-red-700 bg-gradient-to-t text-white font-bold border border-solid border-black hover:from-red-500 hover:to-red-600"
              ...
              <div>
                <button
                  className="from-green-600 to-green-700 bg-gradient-to-t text-white font-bold border border-solid border-black hover:from-green-500 hover:to-green-600"
      ...
      {finishedEditing ? (
        <div className="w-[40%] h-[60px] flex justify-center items-center text-black-500 mt-4 bg-green-300 border-2 border-green-500 border-dashed rounded">
          <Image
            src="/library-icons/checkmark.png"
            alt="Success"
            className="w-[20px] h-[20px] mr-2"
            width={20}
            height={20}
