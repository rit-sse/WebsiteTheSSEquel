...
/**
 * Physics-based per-letter animations — dropped in from the prototype on
 * user's Desktop. Each letter picks an animation by index modulo 8 and
 * fires it on hover. Used on the "accept your nomination" screen so the
 * big celebratory headline feels alive.
 */
...
const DancingLetters = ({
  ...
  return (
    <motion.div
      className={cn(
        "flex flex-wrap items-center justify-center select-none overflow-visible gap-[0.25em]",
        className
      )}
              ...
              <motion.span
                ...
                onAnimationComplete={(definition) => {
                  if (definition === "active")
                    handleAnimationComplete(globalIdx);
                }}
                className={cn(
                  "relative inline-block cursor-pointer font-bold",
                  letterClassName,
                  isActive ? "z-10" : "z-0"
                )}
                style={{
