...
export type PhotoUploaderEvent = {
  id: string;
  title: string;
  date: string;
};
...
export function PhotoBatchUploader({
  ...
  categories,
  mode = "officer",
  uploadInitEndpoint = "/api/photos/uploads",
  completeEndpointForBatch = (batchId) =>
    `/api/photos/batches/${batchId}/complete`,
  onComplete,
}: {
  events: PhotoUploaderEvent[];
  categories: string[];
  mode?: "officer" | "request";
  uploadInitEndpoint?: string;
  completeEndpointForBatch?: (batchId: string) => string;
  ...
  async function uploadBatch() {
    ...
    try {
      ...
      const plan: UploadInitResponse = await response.json();
      ...
      const completeResponse = await fetch(
        completeEndpointForBatch(plan.batchId),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...(isRequestMode
              ? {
                  requestToken: plan.requestToken,
      ...
      toast.success(
        isRequestMode
          ? `Submitted ${createdCount} photo request${
              createdCount === 1 ? "" : "s"
            }.`
      ...
      if (failedCount > 0) {
        toast.error(
          isRequestMode
            ? `${failedCount} photo requests failed to save.`
            : `${failedCount} photo records failed to save.`,
        );
      }
      clearAll();
      if (isRequestMode) {
        ...
        <Card depth={2} className="p-5 md:p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Photos stay private until an officer reviews and approves them.
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="photo-request-name">Name</Label>
              <Input
                id="photo-request-name"
                value={submitterName}
            ...
            <div className="space-y-2">
              <Label htmlFor="photo-request-email">Email</Label>
              <Input
                id="photo-request-email"
                type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="photo-request-note">Note</Label>
            <Input
              id="photo-request-note"
              value={submitterNote}
              onChange={(event) => setSubmitterNote(event.target.value)}
              disabled={uploading}
              placeholder="Optional context for the historians"
            />
          </div>
          <input
            type="text"
      ...
      <Card depth={2} className="p-5 md:p-6 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {isRequestMode
              ? "Optional, applied to every requested photo"
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            optimized WebP copies are prepared for the gallery.
          </p>
        </div>
      </button>
              ...
              >
                {uploading ? (
                  <>
                    ...
                    {isRequestMode ? "Submitting…" : "Uploading…"}
                  </>
                ) : (
                  <>
