...
export const PhotoCarouselPropsSchema = z.object({
  ...
  aspectRatio: z.enum(["16:9", "4:3", "3:2", "1:1", "4:5", "21:9"]).default("16:9"),
...
export const TestimonialRotatorPropsSchema = z.object({
  sources: z.array(z.enum(["quotes", "alumni"])).min(1).default(["quotes", "alumni"]),
  count: z.number().int().min(1).max(20).default(5),
  intervalMs: z.number().int().min(3000).max(20000).default(8000),
});
export type TestimonialRotatorProps = z.infer<typeof TestimonialRotatorPropsSchema>;
...
export const OfficerListingPropsSchema = z.object({
  positionCategory: z.enum(["PRIMARY_OFFICER", "SE_OFFICE", "ALL"]).default("PRIMARY_OFFICER"),
...
export const CtaPropsSchema = z.object({
  ...
  variant: z.enum(["orange", "blue", "pink", "green", "neutral"]).default("orange"),
  align: z.enum(["left", "center", "right"]).default("left"),
});
...
export const BlockNodeSchema = z.discriminatedUnion("type", [
  z.object({ id: z.string(), type: z.literal("heading"), props: HeadingPropsSchema }),
  z.object({ id: z.string(), type: z.literal("markdown"), props: MarkdownPropsSchema }),
  z.object({ id: z.string(), type: z.literal("image"), props: ImagePropsSchema }),
  z.object({ id: z.string(), type: z.literal("photoCarousel"), props: PhotoCarouselPropsSchema }),
  z.object({ id: z.string(), type: z.literal("photoGrid"), props: PhotoGridPropsSchema }),
  z.object({ id: z.string(), type: z.literal("eventFeed"), props: EventFeedPropsSchema }),
  z.object({ id: z.string(), type: z.literal("testimonialRotator"), props: TestimonialRotatorPropsSchema }),
  z.object({ id: z.string(), type: z.literal("projectList"), props: ProjectListPropsSchema }),
  z.object({ id: z.string(), type: z.literal("officerListing"), props: OfficerListingPropsSchema }),
  z.object({ id: z.string(), type: z.literal("sponsorWall"), props: SponsorWallPropsSchema }),
  z.object({ id: z.string(), type: z.literal("zCardRow"), props: ZCardRowPropsSchema }),
  z.object({ id: z.string(), type: z.literal("divider"), props: DividerPropsSchema }),
  z.object({ id: z.string(), type: z.literal("cta"), props: CtaPropsSchema }),
  z.object({ id: z.string(), type: z.literal("heroSection"), props: HeroSectionPropsSchema }),
  z.object({ id: z.string(), type: z.literal("rawHtml"), props: RawHtmlPropsSchema }),
]);
...
export type BlockCategory = "primitive" | "media" | "dynamic" | "officer-only";
...
export const BLOCK_META: { [K in BlockType]: BlockMeta<K> } = {
  heading: {
    ...
    category: "primitive",
    defaultProps: { text: "Section title", level: 2, align: "left" },
  },
  markdown: {
    type: "markdown",
    label: "Text",
  ...
  image: {
    ...
    category: "primitive",
    defaultProps: { src: "", alt: "", caption: "", widthFraction: 1, rounded: true },
  },
  divider: {
    type: "divider",
    label: "Divider",
  ...
  cta: {
    ...
    category: "primitive",
    defaultProps: { text: "Learn more", href: "/", variant: "orange", align: "left" },
  },
  photoCarousel: {
    type: "photoCarousel",
  ...
  photoGrid: {
    ...
    category: "media",
    defaultProps: { categorySlug: "general", count: 12, columns: 4, order: "newest" },
  },
  zCardRow: {
    type: "zCardRow",
    ...
    category: "media",
    defaultProps: { items: [{ imageSrc: "", imageAlt: "", title: "Card title", body: "" }] },
  },
  heroSection: {
    type: "heroSection",
    label: "Hero section",
...
export function validateSlug(input: string): { ok: true; slug: string } | { ok: false; error: string } {
  ...
  if (slug.length > 200) return { ok: false, error: "Slug must be 200 characters or fewer." };
  if (!SLUG_REGEX.test(slug)) {
    return {
      ok: false,
      error: "Slug must be lowercase letters, digits, hyphens, and slashes only (e.g. about/get-involved).",
    };
  }
  const firstSegment = slug.split("/")[0]!;
  for (const prefix of RESERVED_SLUG_PREFIXES) {
    if (firstSegment === prefix || firstSegment.startsWith(prefix)) {
      return { ok: false, error: `"${firstSegment}" is reserved for an existing route.` };
    }
  }
  return { ok: true, slug };
}
 ...
 */
export function autoDescribe(content: PageContent | null | undefined, fallback: string): string {
