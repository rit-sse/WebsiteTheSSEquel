...
async function main() {
  ...
  if (activeOfficer) {
    console.error(
      `❌ ERROR: Position "${positionTitle}" already has an active officer: ${activeOfficer.user.email}`
    );
  ...
  if (existingInvitation) {
    console.log(
      `ℹ️  An invitation already exists for ${email} as ${positionTitle}`
    );
  ...
  if (!inviter) {
    console.log(
      "⚠️  No users exist yet. Creating bootstrap invitation without inviter tracking."
    );
    ...
    console.log(
      "1. Go to the website and sign in with your RIT Google account"
    );
    console.log(
      "2. Run this script again - it will then create a proper invitation"
    );
    process.exit(0);
  }
